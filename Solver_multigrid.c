...
void project_scalar_e(struct All_variables *E, int start_lev, float *AU, float *AD)
{
	...
	int el, e;
	//float average, w;
	/*float average; for arithetic mean out MAJ 9/1/07*/
	double average; /* for geometric mean in MAJ 9/1/07 */
	...
	//const int dims = E->mesh.nsd;
	const int ends = enodes[E->mesh.nsd];
	const double weight = 1.0 / (double)ends;
	//const int vpts = vpoints[E->mesh.nsd];
	//const int n_minus = nels_minus * vpts;

	for(el = 1; el <= nels_minus; el++)
	{
		average = 0.0; /*for arithmetic mean and harmonic mean MAJ 9/1/07  */  
		/*average = 1.0;*/ /*for geometric mean in MAJ 9/1/07 */
		for(i = 1; i <= ends; i++)
		{
			e = E->EL[sl_minus][el].sub[i];
			average += (double)AU[e]; /* for arithmetic mean out MAJ 9/1/07 */
			/*average *= (double)AU[e];*/ /*for geometric mean in MAJ 9/1/07 */
			/*average += 1.0/((double)AU[e]);*/ /*for harmonic mean in MAJ 11/29/07 */
		}
		AD[el] = (float)(average * weight); /*out for arithmetic mean MAJ 9/1/07 */
 		/*AD[el] = (float)pow(average,weight);*/ /* for geometric mean in MAJ 9/1/07 */
 		/*AD[el] = (float)ends/(float)average;*/ /* for harmonic mean in MAJ 11/29/07 */
	}
...
void project_scalar(struct All_variables *E, int start_lev, float *AU, float *AD)
{
	...
	int el, node, node1;
	float w;
	double average; /* changed from float to double 7/12/2006 MAJ*/
	...
	for(el = 1; el <= nels_minus; el++)
		for(i = 1; i <= ends; i++)
		{
			average = 0.0; 		/* For arithmetic mean and harmonic mean. */
			/*average = 1.0;*/		/* Changed from 0.0 to 1.0 for use with Geometric Mean 7/9/07 MAJ */
			node1 = E->EL[sl_minus][el].sub[i];
			for(j = 1; j <= ends; j++)
			{
				node = E->IEN[start_lev][node1].node[j];
				average += (double)AU[node];  /*Out arithmetic mean 6/19/2007 post Gordon*/
				/*average *= (double)AU[node];*/  /* geometric mean. In 6/19/2007 post Gordon*/
				/*average += 1.0/((double)AU[node]);*/ /*for harmonic mean in MAJ 11/29/07 */
			}
			w = (float)(weight * average);  /*Out arithmetic mean 6/19/2007 post Gordon*/
			/*w = (float)pow(average,weight);*/  /* geometric mean. In 6/19/2007 post Gordon*/
 			/*w = (float)ends/(float)average;*/ /* for harmonic mean in MAJ 11/29/07 */

			node = E->IEN[sl_minus][el].node[i];
			
			/* Does the AD need to get changed?? as well? MAJ 6/19/2007*/
...
void project_viscosity(struct All_variables *E)
{
	...
	{
		sl_minus = lv - 1;
		/* MIB Shijie usually uses this one 6/19/2007 post Gordon */
		...
		/* insert line here to print out viscosity for lv. use been here flag
		   or something like that, so it only happens the first time this is called.
		*/
