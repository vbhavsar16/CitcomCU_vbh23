	...
	for(i = 1; i <= E->mesh.nsd; i++)
	{
		...
		for(k = 1; k <= 2; k++)
			if(E->parallel.NUM_PASS[levmax].bound[i][k])
			{
				...
				{
					idb++;
					//	fprintf(E->fp,"Isend d20 me %d, i %d k %d NUM_NEQ %d target_proc %d mst %d\n",E->parallel.me,i,k,
					//	E->parallel.NUM_NEQ[lev].pass[i][k], target_proc, E->parallel.mst[E->parallel.me][target_proc][k]); 
					MPI_Isend(S[k], E->parallel.NUM_NEQ[lev].pass[i][k], MPI_DOUBLE, target_proc, 
						  E->parallel.mst[E->parallel.me][target_proc][k], MPI_COMM_WORLD, &request[idb - 1]);
		...
		for(k = 1; k <= 2; k++)
			if(E->parallel.NUM_PASS[levmax].bound[i][k])
			{
				...
				{
					idb++;
					//	fprintf(E->fp,"Irecv d20 me %d, i %d k %d NUM_NEQ %d target_proc %d mst %d\n",E->parallel.me,i,k,
					//	E->parallel.NUM_NEQ[lev].pass[i][k], target_proc, E->parallel.mst[E->parallel.me][target_proc][k]); 
					MPI_Irecv(R[k], E->parallel.NUM_NEQ[lev].pass[i][k], MPI_DOUBLE, target_proc, 
						  E->parallel.mst[E->parallel.me][target_proc][k], MPI_COMM_WORLD, &request[idb - 1]);
		  ...
		  if(target_proc != E->parallel.me)
		    {
		      ...
		      MPI_Isend(S[k], E->parallel.NUM_NODE[lev].pass[i][k], MPI_FLOAT, 
				target_proc, E->parallel.mst[E->parallel.me][target_proc][k], 
				MPI_COMM_WORLD, &request[idb - 1]);
		    }
		}					/* for k */
