...
struct Parallel
{
	...
	int mst[800][800][3]; /*updated these 4 lines from 100 proc to 400 proc 7/7/08 MAJ to 800 9/9/12 */
	int mst1[800][800];
	int mst2[800][800];
	int mst3[800][800];
...
{								/* general information concerning the fe mesh */
	...
	char gridfile[4][100];
        char meshfile[100];  /* MIB JAN07 */
        int from_file; /* MIB JAN07 */
...
struct MONITOR
{
	...
	int solution_cycles;
	float begin_time;	/* Added 9/9/12 MAJ. for timing on new Lonestar. */
...
struct CONTROL
{
	...
	int output_sdepvnewt_stepzero; /* MIB, JAN07 */
...
#include "advection.h"
	...
	double *P, *F, *H, *S, *U;
  float *stress;  /* note this is only used for the surface stress */
  float *dstress;  /* MIB, JAN07 domain stress */
	float *Psi;
	float *NP;
	float *heatflux;
	float *heatflux_adv;
